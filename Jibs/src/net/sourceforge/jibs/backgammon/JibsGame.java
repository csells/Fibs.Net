...
public class JibsGame {
    ...
    public JibsGame(JibsServer jibsserver, Player playerA, Player playerB,
                    Player player1, Player player2, int Matchlength,
                    int player1Points, int player2Points, JibsMatch matchVersion) {
        this.jibsServer = jibsserver;
        this.playerA = playerA;
        this.playerB = playerB;
        this.board = new BackgammonBoard(jibsserver, player1.getName(),
                                         player2.getName(), Matchlength,
                                         matchVersion);
        this.board.setPlayerX(player1);
        this.board.setPlayerO(player2);
    }
    ...
    public static JibsGame constructGame(JibsServer jibsServer,
        ...
        BackgammonBoard board = new BackgammonBoard(jibsServer,
                                                    resumeData.getPlayer_A(),
                                                    resumeData.getPlayer_B(),
                                                    resumeData.getMatchlength()
                                                              .intValue(),
        ...
        game.getBackgammonBoard()
            .setMatchVersion(new JibsMatch(resumeData.getMatchVersion()));

        Die d1 = new Die(jibsServer);
        Die d2 = new Die(jibsServer);

        game.getBackgammonBoard().setpDie1(d1);
        game.getBackgammonBoard().setpDie2(d2);
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            case 33: // player dice
                help = Integer.valueOf(x);
                board.getpDie1().setValue(help);

                break;

            case 34: // player dice
                help = Integer.valueOf(x);
                board.getpDie2().setValue(help);

                break;

            case 35: // opp dice
                help = Integer.valueOf(x);
                board.getoDie1().setValue(help);

                break;

            case 36: // opp dice
                help = Integer.valueOf(x);
                board.getoDie2().setValue(help);
    ...
    public static void deleteGames(JibsServer jibsServer, Player player1,
        ...
        try {
            ...
            sqlClient.delete("SavedGames.deleteSavedGame", savedGame);
    ...
    public static SavedGameParam loadGame(JibsServer jibsServer, String name1,
                                          String name2) {
        SqlMapClient sqlMapClient = jibsServer.getSqlMapInstance();
        SavedGameParam savedGame = new SavedGameParam(name1, name2);
        SavedGameParam retSavedGame = null;

        try {
            retSavedGame = (SavedGameParam) sqlMapClient.queryForObject("SavedGames.loadSavedGame",
                                                                        savedGame);
        } catch (SQLException e) {
            jibsServer.logException(e);
        }
    ...
    public int calcPossibleMoves(JibsGame game, Player player,
                                 boolean isFirstMove, int turn,
                                 MoveChances nrChances, JibsWriter out,
                                 JibsWriter out2, BackgammonBoard board,
                                 int dice1, int dice2) {
        Object[] obj = null;
        String msg = null;
        int nrMoves = 0;
        ...
        case -1: // X
            ...
            board.getPlayerO()
                 .show2WatcherBoard(opBoard, board.getPlayerO().getName(), 0,
                                    0, dice1, dice2);
            nrMoves = nrChances.calcPossibleMovesX(turn, dice1, dice2);
            board.setCanMove(nrMoves);
            board.outBoard(out, "You", turn, dice1, dice2, 0, 0); // X
            board.getPlayerX()
                 .show2WatcherBoard(board, board.getPlayerX().getName(), dice1,
                                    dice2, 0, 0);
        ...
        case 1: // O
            ...
            board.setCanMove(0);
            board.outBoard(out, "You", turn, 0, 0, dice1, dice2); // X
            board.getPlayerX()
                 .show2WatcherBoard(board, board.getPlayerX().getName(), 0, 0,
                                    dice1, dice2);
            board.setCanMove(nrMoves);
            opBoard = board.switch2O();
            opBoard.outBoard(out2, "You", turn, dice1, dice2, 0, 0); // O
            board.getPlayerO()
                 .show2WatcherBoard(opBoard, board.getPlayerO().getName(),
                                    dice1, dice2, 0, 0);
        ...
        /*
         * System.err.println("----------------------------"); Iterator iter =
         * col.iterator(); while(iter.hasNext()) { PossibleMove pMove =
         * (PossibleMove) iter.next(); System.err.println(pMove.toString()); }
         */
        return nrMoves;
    }
    ...
    public void resumeGame(SavedGameParam resumeData, int startTurn, Die dice1,
                           Die dice2, int player1Points, int player2Points) {
        ...
        case 1:
            ...
            calcPossibleMoves(this, board.getPlayerX(), true, board.getTurn(),
                              nrChances, out1, out2, board, dice1.getValue(),
                              dice2.getValue());
    ...
    public int rollO(JibsWriter out, JibsWriter out2, Player player,
        ...
        Die dice11 = board.getoDie1();
        Die dice12 = board.getoDie2();
        dice11.roll();
        dice12.roll();
        board.setoDie1(dice11);
        board.setoDie2(dice12);

        // m_you_roll=You roll %0 and %1.
        Object[] obj = new Object[] {
                           Integer.valueOf(dice11.getValue()),
                           Integer.valueOf(dice12.getValue())
                       };
        String msg = jibsServer.getJibsMessages().convert("m_you_roll", obj);
        player.getOutputStream().println(msg);
        // m_other_roll=%0 rolls %1 and %2.
        obj = new Object[] {
                  player.getName(), Integer.valueOf(dice11.getValue()),
                  Integer.valueOf(dice12.getValue())
              };
        ...
        int nrMoves = calcPossibleMoves(this, player, false, turn, nrChances,
                                        out, out2, bgBoard, dice11.getValue(),
                                        dice12.getValue());
    ...
    public int rollX(JibsWriter out, JibsWriter out2, Player player,
                     BackgammonBoard bgBoard, int turn) {
        Die dice11 = board.getpDie1();
        Die dice12 = board.getpDie2();
        dice11.roll();
        dice12.roll();

        board.setpDie1(dice11);
        board.setpDie2(dice12);
        ...
        try {
            if (true) {
                ...
                SavedGameParam savedGame = new SavedGameParam(this,
                                                              ...
                                                              board.getPlayerO(),
                                                              getBackgammonBoard(),
                                                              board.getMatchlength(),
                                                              board.getTurn(),
                                                              board.getMatchVersion(),
                                                              new Date());
                sqlMap.insert("SavedGames.saveGame", savedGame);
    ...
    public void startGame(int startTurn, Die dice1, Die dice2,
                          int player1Points, int player2Points, int mayDouble1,
                          int mayDouble2) {
        ...
        case 1:
            nrMoves = calcPossibleMoves(this, board.getPlayerO(), true,
                                        board.getTurn(), nrChances, out1, out2,
                                        board, dice1.getValue(),
                                        dice2.getValue());
    ...
    public void winGameO(JibsGame game, BackgammonBoard board, int winPoints) {
        ...
        default:
            ...
            out1.println(msg);
            obj = new Object[] { winPlayer.getName(), Integer.valueOf(winPoints) };
        ...
        if (gamePoints >= board.getMatchlength()) {
            ...
            board.getPlayerX().changeRating(newRating, newExperience);

            newRating = ratingB + d;
            newExperience = expB + board.getMatchlength();

            board.getPlayerO().changeRating(newRating, newExperience);
            out1.println(winMsg);
            out2.println(looseMsg);
            new GameEnder(jibsServer, board.getPlayerX(), board.getPlayerO()).start();
        }
    }
